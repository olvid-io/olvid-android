...
public class GroupMemberSuppressionDialogFragment extends DialogFragment {
    private static final String BYTES_OWNED_IDENTITY_KEY = "bytes_owned_identity";
    private static final String BYTES_GROUP_OWNER_AND_UID_KEY = "bytes_group_owner_and_uid";
    private byte[] bytesOwnedIdentity;
    private byte[] bytesGroupUid;
    private List<Contact> selectedContacts = null;
    private Group group = null;

    public static GroupMemberSuppressionDialogFragment newInstance(byte[] bytesOwnedIdentity, byte[] bytesGroupOwnerAndUid) {
        GroupMemberSuppressionDialogFragment fragment = new GroupMemberSuppressionDialogFragment();
        Bundle args = new Bundle();
        args.putByteArray(BYTES_OWNED_IDENTITY_KEY, bytesOwnedIdentity);
        args.putByteArray(BYTES_GROUP_OWNER_AND_UID_KEY, bytesGroupOwnerAndUid);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (arguments != null) {
            ...
            bytesGroupUid = arguments.getByteArray(BYTES_GROUP_OWNER_AND_UID_KEY);
        }
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        dialogTitle.setText(R.string.dialog_title_remove_group_members);
        ...
        okButton.setOnClickListener((View view) -> {
            ...
            final byte[][] bytesRemovedMemberIdentities = new byte[selectedContacts.size()][];
            ...
            for (Contact selectedContact: selectedContacts) {
                ...
                bytesRemovedMemberIdentities[i] = selectedContact.bytesContactIdentity;
                i++;
            }


            final AlertDialog.Builder builder = new SecureAlertDialogBuilder(requireContext(), R.style.CustomAlertDialog)
                    .setTitle(R.string.dialog_title_remove_group_members)
                    .setMessage(getResources().getQuantityString(R.plurals.dialog_message_remove_group_members, selectedContacts.size(), selectedContacts.size(), group.getCustomName(), sb.toString()))
                    .setPositiveButton(R.string.button_label_ok, (DialogInterface dialog, int which) -> {
                        try {
                            AppSingleton.getEngine().removeContactsFromGroup(bytesOwnedIdentity, bytesGroupUid, bytesRemovedMemberIdentities);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    })
        ...
        filteredContactListFragment.setUnfilteredContacts(Transformations.switchMap(AppDatabase.getInstance().groupDao().getLiveData(bytesOwnedIdentity, bytesGroupUid), (Group liveGroup) -> {
            ...
            group = liveGroup;
            return AppDatabase.getInstance().contactDao().getAllInGroupOrPending(bytesOwnedIdentity, bytesGroupUid);
        }));
