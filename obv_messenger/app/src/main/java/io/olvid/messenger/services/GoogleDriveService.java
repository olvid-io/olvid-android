...
public class GoogleDriveService {
    ...
    private static final String BACKUP_FILE_NAME_MODEL_PART =  Build.BRAND + " " + Build.MODEL;

    public static void uploadBackupToDrive(byte[] backupContent, Runnable onSuccessCallback) {
        App.runThread(() -> {
            GoogleSignInAccount account = GoogleSignIn.getLastSignedInAccount(App.getContext());
            if (account == null) {
                if (SettingsActivity.useAutomaticBackup()) {
                    App.openAppDialogBackupRequiresDriveSignIn();
                }
                return;
            }
            ...
            try {
                ...
                String fileName = deviceUniqueId + "|" + BACKUP_FILE_NAME_MODEL_PART;
    ...
    public static void listBackupsOnDrive(OnBackupsListedCallback onBackupsListedCallback) {
        App.runThread(() -> {
            GoogleSignInAccount account = GoogleSignIn.getLastSignedInAccount(App.getContext());
            if (account == null) {
                onBackupsListedCallback.onFailure(OnBackupsListedCallback.ERROR_NO_ACCOUNT);
                return;
            }
            try {
                GoogleAccountCredential credential = GoogleAccountCredential.usingOAuth2(App.getContext(), Collections.singleton(DriveScopes.DRIVE_APPDATA));
                ...
                List<File> folderList = googleDriveService.files().list()
                        ...
                        .execute().getFiles();
                if (folderList.size() == 0) {
                    onBackupsListedCallback.onSuccess(new ArrayList<>());
                    return;
                }
                ...
                List<OnBackupsListedCallback.BackupItem> backupsList = new ArrayList<>();
                for (File file: fileList) {
                    String[] parts = file.getName().split("\\|");
                    if (parts.length == 2) {
                        backupsList.add(new OnBackupsListedCallback.BackupItem(parts[1], file.getId(), file.getModifiedTime().getValue()));
                    }
                }
                Collections.sort(backupsList, Collections.reverseOrder());
                onBackupsListedCallback.onSuccess(backupsList);
                return;
            } catch (Exception e) {
                e.printStackTrace();
            }
            onBackupsListedCallback.onFailure(OnBackupsListedCallback.ERROR_UNKNOWN);
        });
    }


    public static void downloadBackupFromDrive(String fileId, OnBackupDownloadCallback onBackupDownloadCallback) {
        App.runThread(() -> {
            GoogleSignInAccount account = GoogleSignIn.getLastSignedInAccount(App.getContext());
            if (account != null) {
                try {
                    ...
                    onBackupDownloadCallback.onSuccess(baos.toByteArray());
                    return;
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            onBackupDownloadCallback.onFailure();
        });
    }

    public interface OnBackupDownloadCallback {
        void onSuccess(byte[] backupContent);
        void onFailure();
    }

    public interface OnBackupsListedCallback {
        int ERROR_UNKNOWN = 1;
        int ERROR_NO_ACCOUNT = 2;

        void onSuccess(List<BackupItem> backupTimestampAndNames);
        void onFailure(int error);

        class BackupItem implements Comparable<BackupItem> {
            public final String fileName;
            public final String fileId;
            public final long timestamp;

            public BackupItem(String fileName, String fileId, long timestamp) {
