...
public class EmergencyPINCreationDialogFragment extends DialogFragment implements View.OnClickListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View dialogView = inflater.inflate(R.layout.dialog_fragment_emergency_pin_creation, container, false);
    ...
    private void validatePINs() {
        ...
        if (firstPIN.length() < 4) {
            ...
            for (int i=3; i<firstPIN.length(); i++) {
                String sub = firstPIN.substring(0, i+1);
                if (SettingsActivity.verifyPIN(sub)) {
                    // the actual PIN is a prefix of the entered emergency PIN
                    if (usePassword) {
                        textPINErrorMessage.setText(R.string.error_text_password_prefix_of_emergency);
                    } else {
                        textPINErrorMessage.setText(R.string.error_text_pin_prefix_of_emergency);
                    }
                    pinSecondInput.setEnabled(false);
                    createPINButton.setEnabled(false);
                    return;
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.button_create_pin) {
            ...
            String secondPIN = pinSecondInput.getText().toString();
            if (firstPIN.equals(secondPIN)) {
                SettingsActivity.saveEmergencyPIN(firstPIN);
                dismiss();
                App.toast(R.string.toast_message_emergency_pin_set, Toast.LENGTH_SHORT);
