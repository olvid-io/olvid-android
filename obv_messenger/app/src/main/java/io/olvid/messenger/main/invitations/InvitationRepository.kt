...
object InvitationRepository {
    ...
    private val invitationDao = AppDatabase.getInstance().invitationDao()
    ...
    val invitations: LiveData<List<Invitation>> =
        AppSingleton.getCurrentIdentityLiveData().switchMap { ownedIdentity: OwnedIdentity? ->
            if (ownedIdentity == null) {
                return@switchMap MutableLiveData<List<Invitation>>(null)
            }
            invitationDao.getAllForOwnedIdentity(ownedIdentity.bytesOwnedIdentity)
        }
}
