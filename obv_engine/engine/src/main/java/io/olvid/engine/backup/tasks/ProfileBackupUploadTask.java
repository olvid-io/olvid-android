    ...
    public BackupTaskStatus execute() {
        try (BackupManagerSession backupManagerSession = backupManagerSessionFactory.getSession()) {
            ...
            if (profileBackupThreadId == null) {
                ...
                profileBackupThreadId = ProfileBackupThreadId.create(backupManagerSession, ownedIdentity, backupManagerSession.prng);
                if (profileBackupThreadId != null) {
            ...
            if (!backupManagerSession.identityDelegate.isActiveOwnedIdentity(backupManagerSession.session, ownedIdentity)) {
                Logger.w("ProfileBackupUploadTask started for an inactive OwnedIdentity");
                // never backup a profile that is not active
                return BackupTaskStatus.PERMANENT_FAILURE;
            }
            BackupSeed backupSeed = backupManagerSession.identityDelegate.getOwnedIdentityBackupSeed(backupManagerSession.session, ownedIdentity);
            if (backupSeed == null) {
            ...
            try {
                ...
                profileBackupSnapshot = ObvProfileBackupSnapshot.get(ownedIdentity, identityBackupAndSyncDelegate, backupManagerSession.appBackupAndSyncDelegate);
            } catch (Exception e) {
                Logger.x(e);
                return BackupTaskStatus.RETRIABLE_FAILURE;
            } finally {
